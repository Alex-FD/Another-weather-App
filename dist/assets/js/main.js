...
document.addEventListener('DOMContentLoaded',function () {
        ...
        if (!cityName){
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    getElement('deg','id').textContent = 0;
                    console.log('The request is invalid.'); /*TODO show some message to signal that city name is incorrect."*/
                    console.log('Status: '+ xhr.status);
                    console.log('Error: '+ xhr.statusText);
                }
            };
    ...
    function getNavigationData() {
        ...
        function getLocation() {
            ...
            if (navigator.geolocation) {
                ...
                currPosTxt.textContent = "Функция геолокации не поддерживатся браузером.";
            }
        }
        getLocation();
        ...
        function showError(error) {
            switch (error.code) {
                case error.POSITION_UNAVAILABLE:
                    ...
                    break;
                case error.TIMEOUT:
                    currPosTxt.textContent = "Время ожидания информации о геолокации пользователя истекло.";
                    break;
            }
        }
    ...
    function setWindDirection(obj) {
        ...
        if (obj.wind[1].direction === undefined){
            ...
            direction.style.mozTransform = 'rotate(' + obj.wind[1].direction.toFixed(0) + 'deg' + ')';
            direction.style.msTransform = 'rotate(' + obj.wind[1].direction.toFixed(0) + 'deg' + ')';
            direction.style.oTransform = 'rotate(' + obj.wind[1].direction.toFixed(0) + 'deg' + ')';
            direction.style.transform = 'rotate(' + obj.wind[1].direction.toFixed(0) + 'deg' + ')';
        }
    }
    function setBladeRotationTime(obj) {
        let blade = getElement('blade','class',0);
        let bladeRotationTime = parseFloat(window.getComputedStyle(blade).getPropertyValue('animation-duration'));
    ...
    function setWindSpeed(obj) {
        ...
        if (obj.wind[0].speed === undefined){
            ...
            speed.textContent = obj.wind[0].speed.toFixed(1) + ' м/с' + ', ' + defineWindDirection((obj.wind[1].direction).toFixed(0));
        }
    }
    function setPressure(obj) {
        ...
        if (obj.pressure === undefined){
            ...
            pressure.textContent = convertPressure(obj.pressure).toFixed(0) + ' мм рт.ст.';
        }
    }
